...
| 项目化新终端 | 新终端可从常用目录、已索引项目、最近项目或自定义路径启动 | 选择项目会更新默认标题；手动标题不会被覆盖 |
...
“+ 新终端”打开轻量面板，而不是无提示地创建 shell。面板提供四类入口：

- **当前项目**：当前页面的项目筛选有值时，作为默认选择；
- **常用目录**：用户维护 3–5 个项目根目录，用于把已索引项目分组，目录本身也可直接作为 cwd；
- **最近项目**：按最近会话活动排序的已索引项目；
- **其他路径**：用户显式输入本机目录路径，daemon 仍须验证其为目录，失效时不得静默改为其他项目。

选择项目或路径时，标题默认取 cwd 的最后一级目录名；用户修改标题后，该标题成为显式覆盖，后续切换目录不得改写它。创建完成后，shell 的 `cd` 继续更新卡片上的 cwd，但**不自动重命名终端**：标题是用户识别任务的稳定名称，cwd 是当前地点，两者不能混为一谈。

常用目录存于本地 SQLite 设置表，最多 5 条、路径唯一、按用户排序；不扫描或暴露任意文件系统目录。面板中只展示已有索引的项目路径，输入“其他路径”仅用于创建 PTY，且沿用既有本地鉴权与项目禁录策略。
